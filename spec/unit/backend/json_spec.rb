require 'spec_helper'
require 'tmpdir'
require 'hiera/backend/json'
require 'hiera/logger/in_memory'

describe Hiera::Backend::Json do
  it "has no answer when the data file does not exist" do
    backend = Hiera::Backend::Json.new("testing",
                                       { "dir" => "/does/not/exist" },
  ...
  it "has no answer when the data file does not contain the requested key" do
    a_json_file("testing.yaml", { "some_key" => "the_value" }) do |directory|
      backend = Hiera::Backend::Json.new("testing",
                                         { "dir" => directory },
                                         Hiera::Logger::InMemory.new({}),
                                         nil)
  ...
  it "answers with the value found in the yaml file" do
    a_json_file("testing.json", { "some_key" => "the_value" }) do |directory|
      backend = Hiera::Backend::Json.new("testing",
                                         { "dir" => directory },
                                         Hiera::Logger::InMemory.new({}),
  ...
  def a_json_file(name, data, &block)
